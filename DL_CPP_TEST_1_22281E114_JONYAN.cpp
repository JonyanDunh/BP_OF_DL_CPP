...
public:
	MatrixXd forward(MatrixXd x) {
		for (int i = 0; i < x.rows(); i++)
		{
			x.row(i) = x.row(i).array().exp() / x.row(i).array().exp().sum();
...
public:
	...
	void init(int input_size, int hidden_size, int output_size, double weight_init_std)
	{
...
void run_deep_learning(MatrixXd images, MatrixXd labels,int input_size, int hidden_size, int output_size, double weight_init_std, int mini_batch_count, double learning_rate,int learning_times) {
	...
	network.init(input_size, hidden_size, output_size, weight_init_std);
	ReLULayer ReLULayer;
	srand(time(nullptr));
